...
// ====================================================================================
// This file serves as the interface between the application and the D1 database.
// It replaces all Mongoose models and provides async functions for all CRUD operations.
//
// IMPORTANT: You must run the SQL statements below in your Cloudflare dashboard
// or via `wrangler d1 execute` to create the necessary tables.
// ====================================================================================
...
CREATE TABLE IF NOT EXISTS users (
  ...
  subscription_expiresAt TEXT, -- ISO 8601 string
  subscription_providerId TEXT,
  subscription_hasUsedGoldTrial INTEGER DEFAULT 0 NOT NULL, -- 0 for false, 1 for true
...
CREATE TABLE IF NOT EXISTS multilevel_exam_results (
  ...
  feedbackBreakdown TEXT NOT NULL, -- Stored as JSON string
  transcript TEXT NOT NULL, -- Stored as JSON string
  examContent TEXT, -- Stored as JSON string
...
-- D1 automatically removes expired rows if you use TTL, but TTL isn't standard SQL.
-- You might need a separate cleanup worker or handle expiration in the app logic.
...
CREATE TABLE IF NOT EXISTS content_part3 (
  id TEXT PRIMARY KEY,
  topic TEXT NOT NULL,
  forPoints TEXT NOT NULL, -- Stored as JSON string
  againstPoints TEXT NOT NULL, -- Stored as JSON string
  imageUrl TEXT,
  tags TEXT
);
