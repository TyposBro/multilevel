// Enhanced payment controller with improved Click integration

import * as paymentService from "../../services/paymentService";
import { db } from "../../db/d1-client";
import { verifyWebhookSignature, checkInvoiceStatus } from "../../services/providers/clickService";
...
export const createPayment = async (c) => {
  try {
    const { provider, planId, paymentMethod, phoneNumber } = await c.req.json();
    ...
    // For Click, support both web redirect and invoice creation
    if (provider.toLowerCase() === "click") {
      if (paymentMethod === "invoice") {
        if (!phoneNumber) {
          return c.json({ message: "Phone number is required for invoice creation." }, 400);
        }

        const result = await paymentService.createClickInvoice(c, planId, user.id, phoneNumber);
        return c.json(
          {
            success: true,
            provider: "click",
...
/**
 * @desc    Check payment status (useful for invoice payments)
...
export const checkPaymentStatus = async (c) => {
  try {
    ...
    // For Click invoices, check status with Click API
    if (transaction.provider === "click" && transaction.externalId) {
      try {
        externalStatus = await checkInvoiceStatus(c, transaction.externalId);
      } catch (error) {
    ...
    return c.json({ message: "Server error while checking payment status" }, 500);
  }
};
...
export const handleСlickWebhook = async (c) => {
  ...
  // 3. Check for external errors reported by Click
  ...
  // 5. Look up the plan using the service_id from the webhook
  ...
  if (action === 0) {
    ...
    // Update transaction with prepare info
    try {
      await db.updatePaymentTransaction(c.env.DB, transaction.id, {
        status: "PREPARED",
        providerTransactionId: click_trans_id,
      });
      console.log("Transaction status updated to PREPARED");
    } catch (dbError) {
    ...
    if (transaction.status === "FAILED") {
      console.log("Transaction was previously failed");
      return c.json({ error: -9, error_note: "Transaction cancelled" });
    }
