...
describe("Payment Service", () => {
  ...
  describe("verifyPurchase", () => {
    it("should fail for an unsupported provider", async () => {
      const result = await paymentService.verifyPurchase(mockContext, "unknown", "token", mockUser);
    ...
    it("should fail if the provider check returns a falsy value", async () => {
      ...
      const result = await paymentService.verifyPurchase(mockContext, "payme", "token", mockUser);
      expect(result.success).toBe(false);
    ...
    it("should succeed for a valid transaction and update user", async () => {
      paymeService.checkTransaction.mockResolvedValue({ state: 4, planId: "gold_monthly" });
